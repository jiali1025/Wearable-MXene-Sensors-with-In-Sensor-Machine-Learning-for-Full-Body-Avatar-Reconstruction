...
val_x = inputs[-test_portion:]
val_y = labels[-test_portion:]
# print(test_y)
...
label_sc.inverse_transform(labs2)
print(test_x2[0])
...
def evaluate(model, val_x, val_y, label_sc):
    model.eval()
    outputs = []
    targets = []
    start_time = time.clock()
    ...
    for i in range(len(outputs)):
        sMAPE += np.mean(abs(outputs[i] - targets[i]) / targets[i] ) / len(outputs)
...
gru_model.cuda()
gru_outputs_val, targets_val, gru_sMAPE_val = evaluate(gru_model, val_x, val_y, label_sc)
gru_outputs, targets, gru_sMAPE = evaluate(gru_model, test_x2, test_y2, label_sc)
...
np.savetxt("predict_sensor_only_no_cool.csv", final_numpy, delimiter=",")
