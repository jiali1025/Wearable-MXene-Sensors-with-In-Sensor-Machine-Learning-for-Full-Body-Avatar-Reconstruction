# -*- coding: utf-8 -*-
"""
https://colab.research.google.com/drive/1SKoofZWn82hW6ost1KhroLSt3F91Pxr7
"""
...
The CNN model and RNN model. However, this script is only for the training of the RNN model
...
def rnn_evaluate(filters1,filters2,LR,reg,n_hours,epochs):
  ...
  x_train=np.reshape(x_train,[-1,7,n_hours+1])
  x_test=np.reshape(x_test,[-1,7,n_hours+1])
  x_validate=np.reshape(x_validate,[-1,7,n_hours+1])
  RNN_model(filters1,filters2,LR,reg,n_hours,x_train,y_train,x_validate,y_validate,epochs,0)
...
"""#Create a model and tune the hyperparameters. Please skip to the next block if you dont want to tune the hyperparameters"""

rnnBO = BayesianOptimization(rnn_evaluate, {'filters1': (1,500),'filters2': (1,100),'LR': (0.00001,0.01),'reg': (0.1,0.5), 'n_hours': (3,20), 'epochs': (10,10)},verbose=2)
rnnBO.maximize(init_points=10,n_iter=5,acq='poi',kappa=1,xi=1e-1,alpha=1e-6)#exploration
rnnBO.maximize(init_points=0,n_iter=4,acq='poi',kappa=1,xi=1e-4,alpha=1e-6)#exploitation
...
filters1=290
filters2=11
LR= 0.0002082220068102038
n_hours=20
epochs=10
reg=0.32778929280282754
...
RNN_model(filters1,filters2,LR,reg,n_hours,x_train,y_train,x_validate,y_validate,epochs,2)
model = load_model('./best_model.h5')
...
y_pred=label_sc.inverse_transform(y_pred_model)
y_test_orig=label_sc.inverse_transform(y_test)

pyplot.plot(y_pred_model[535,:])
pyplot.plot(y_test[530,:])
print(y_pred[535,:] - y_test_orig[530,:])
...
y_pred_avg=moving_average(y_pred,2)
y_test_tflite = {"x_test":x_test,"y_test_orig":y_test_orig,"y_pred_avg":y_pred_avg,"y_pred":y_pred} #You can download this to create a gif using y_pred_orig or y_pred
...
"""Below this is for hardware specific optimizations
...
def representative_dataset():
  for i in range(50):
    yield([x_train[i,:,:,0].reshape(1,7,n_hours+1).astype(np.float32)])
...
y_pred = (model.predict(x_test)*y_train_std) + y_train_mean
...
y_test_pred_no_quant_tflite=(y_test_pred_no_quant_tflite*y_train_std) + y_train_mean
...
y_test_pred_tflite=(y_test_pred_tflite*y_train_std) + y_train_mean
